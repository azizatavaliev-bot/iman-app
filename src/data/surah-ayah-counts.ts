// Number of ayahs per surah. Index = surah number (1..114). Index 0 is unused.
// Total: 6236 ayahs in the Quran.
...
/** Total number of ayahs in the Quran (sum of AYAH_COUNTS) */
export const TOTAL_AYAHS = 6236;

/** Number of surahs */
export const TOTAL_SURAHS = 114;

/** Sum ayahs across a list of read surah numbers */
export function sumAyahsForSurahs(surahNumbers: number[]): number {
  let total = 0;
  for (const n of surahNumbers) {
    if (n >= 1 && n <= 114) total += AYAH_COUNTS[n];
  }
  return total;
}
