    ...
    <AudioContext.Provider value={contextValue}>
      ...
      {showAudioUnlockOverlay && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowAudioUnlockOverlay(false);
          }}
        >
          <div className="glass-card max-w-sm w-full p-8 text-center border border-emerald-500/20 relative">
            {/* Закрыть — чтобы нельзя было застрять на этом экране */}
            <button
              onClick={() => setShowAudioUnlockOverlay(false)}
              aria-label="Закрыть"
              className="absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center text-slate-400 hover:text-white active:bg-white/10 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <button
              onClick={() => setShowAudioUnlockOverlay(false)}
              className="w-full py-2.5 mt-2 text-sm text-slate-400 hover:text-slate-200 transition-colors"
            >
              Позже
            </button>
